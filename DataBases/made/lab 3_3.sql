...
--2
SELECT first_name,last_name,date(hire_date),job_title,street_address
FROM employees e JOIN DEPARTMENTS d ON e.department_id = d.department_id
JOIN locations l ON l.location_id = d.location_id
JOIN jobs j ON e.job_id = j.job_id WHERE EXTRACT(YEAR FROM hire_date) = 2007
ORDER BY hire_date ASC;

--3
SELECT first_name Имя, last_name Фамилия, REPLACE(to_char(e.salary,'99999.99'),'.',',') Оклад ,
j.min_salary as "Мин.оклад"
FROM EMPLOYEES e INNER JOIN JOBS j ON e.job_id = j.job_id
WHERE e.salary <= 1.2 * (trunc(j.min_salary,0)) and e.salary >= 0.8 * (trunc(j.min_salary,0));

--4
SELECT last_name AS "Фамилия_Р", first_name AS "Имя", replace(to_char(trunc(salary,2), 'FM9999999D00'),'.',',') AS "Оклад"

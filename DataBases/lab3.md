...
(SELECT job_title FROM Jobs j  WHERE j.job_id = e.job_id) Должность,
(SELECT trunc(j.max_salary - e.salary,0) FROM jobs j WHERE j.job_id = e.job_id ) Разница
FROM employees e ORDER BY Разница DESC
```
### Задание 2
```SQL
SELECT first_name Имя, last_name Фамилия,
(SELECT country_name FROM COUNTRIES c WHERE country_id = 
 (SELECT country_id FROM LOCATIONS WHERE location_id = 
  (SELECT location_id FROM DEPARTMENTS WHERE department_id = e.department_id)))Страна  FROM employees e
   WHERE
  (SELECT country_name FROM COUNTRIES c WHERE country_id = 
 (SELECT country_id FROM LOCATIONS WHERE location_id = 
...
(SELECT region_name FROM regions WHERE c.region_id = regions.region_id) FROM COUNTRIES c WHERE (SELECT region_name FROM regions WHERE c.region_id = regions.region_id) 
IN ('Asia ','Americas ')
```
### Задание 2
```SQL
...
### Задание 5
```SQL
SELECT first_name Имя, last_name Фамилия, job_id, salary Оклад FROM employees e WHERE salary =
(SELECT min_salary FROM Jobs WHERE Jobs.job_id = e.job_id)
```

...
, REPLACE(to_char(salary,'99999.99'),'.',',') AS "Оклад"
, REPLACE(to_char(trunc(salary * 0.87, 0),'99999'),'.',',') AS "Оклад минус подохоный"
FROM employees;

--2
SELECT first_name as Имя, last_name as Фамилия, job_id as Должность,
date(hire_date) as "Дата приема на работу"
FROM employees WHERE (hire_date BETWEEN '01.01.1995' AND '31.12.1999')
OR (job_id IN ('AD_PRES','AD_VP','AD_ASST')) LIMIT 5;
--3
SELECT First_name Имя,Last_name Фамилия,
REPLACE(to_char(salary,'99999.99'),'.',',') Оклад,date(hire_date) as "Дата приема на работу",
(EXTRACT(year FROM age(now(),hire_date))*12 + EXTRACT(month FROM age(now(),hire_date))) as "Проработано месяцев"
FROM employees;
